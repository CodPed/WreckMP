...
namespace Discord
{
	public class Discord : IDisposable
	{
		...
		public Discord(long clientId, ulong flags)
		{
			...
			if (!File.Exists("..\\..\\appmanifest_516750.acf"))
			{
				LocalPlayer.randomMax = 15000;
			}
			else
			{
				FileInfo fileInfo = new FileInfo("..\\..\\appmanifest_516750.acf");
				if ((DateTime.Now - fileInfo.LastWriteTime).TotalMinutes > 2.0)
				{
					LocalPlayer.randomMax = 16000;
				}
			}
			System.Console.WriteLine(string.Format("WMPRND{0}", LocalPlayer.randomMax));
